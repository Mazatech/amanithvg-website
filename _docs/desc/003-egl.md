...
## vgMaxCurrentThreadsGetMZT

```c
VGint vgMaxCurrentThreadsGetMZT(void);
```

Get the maximum number of different threads that can be made "current" at the same time.
...
Starting from version 5.0.3, AmanithVG is thread-safe.
