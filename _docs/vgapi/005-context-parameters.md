...
| `VG_BLEND_MODE` | `VGBlendMode` | `VG_BLEND_SRC`<br><span class="ovg_default">`VG_BLEND_SRC_OVER`</span><br>`VG_BLEND_DST_OVER`<br>`VG_BLEND_SRC_IN`<br>`VG_BLEND_DST_IN`<br>`VG_BLEND_MULTIPLY` `VG_BLEND_SCREEN`<br>`VG_BLEND_DARKEN`<br>`VG_BLEND_LIGHTEN`<br>`VG_BLEND_ADDITIVE` |
